...
Các phương pháp trên có nhược điểm là chi phí thiết bị cao, chi phí tính toán lớn và thuật toán phức tạp do robot phải xử lý lượng lớn dữ liệu đầu vào. Trong luận văn này, để đảm bảo robot có thể hoạt động được theo thời gian thực và tích hợp trên hệ thống sẵn có, tác giả sử dụng phương pháp tích hợp thêm một tầng cảm biến khoảng cách hồng ngoại để phối hợp với Lidar và cảm biến siêu âm có sẵn trên robot, hỗ trợ và tăng cường tránh vật cản cho robot. Tác giả thực hiện một số nhiệm vụ như sau:

\begin{itemize}
    \item Thiết kế mạch lấy dữ liệu cảm biến
    \item Xử lý dữ liệu từ các cảm biến khoảng cách hồng ngoại
    \item Ứng dụng và phát triển giải thuật điều khiển tránh vật cản bằng hệ thống cảm biến khoảng cách hồng ngoại.
    \item Phối hợp phân quyền điều khiển robot
...
Robot sử dụng hai bánh dẫn động chính và di chuyển trên mặt phẳng, do vậy chuyển động của robot được quy về hai thông số vận tốc dài $v$ và vận tốc góc $\omega$ theo phương pháp ước tính trạng thái robot dead-reckoning Hình \ref{fig:mobile-rb-dead-reckoning} \cite{pyo2017ros}.
...
\begin{figure}[htbp]
    ...
    \caption[Cơ chế hoạt động của cảm biến khoảng cách hồng ngoại]{Cơ chế hoạt động của cảm biến khoảng cách hồng ngoại \cite{Heikopikner2015}}
...
\begin{figure}[htbp]
    ...
    \caption[Mối liên hệ giữa khoảng cách và điện áp của cảm biến IR Sharp GP2Y0A21YK0F]{Mối liên hệ giữa khoảng cách và điện áp của cảm biến IR Sharp GP2Y0A21YK0F \cite{irdatasheet}}
    \label{fig:irSharp-vol-distance}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/arg_obstacle-detection-area1.png}
...
\textbf{Giải thuật vùng khẩn cấp U}:
...
\begin{figure}[htbp]
    \centering
    \subfloat[]{\includegraphics[width=\linewidth]{figures/BB-arg.png}} \\
...
Trong luận văn này, tác giả sử dụng \codeword{cmd_vel_mux} của \codeword{nodelet} để thực hiện phân quyền điều khiển cho robot để phối hợp điều khiển từ hệ thống cảm biến an toàn IR vào hệ thống điều khiển robot. (Hình \ref{fig:phanquen-dexuat})
...
% Trong phần này, sau khi phát triển xong hệ thống phát hiện và điều khiển tránh vật cản bằng cảm biến hồng ngoại, tác giả sẽ tích hợp vào trong hệ thống điều khiển robot.
